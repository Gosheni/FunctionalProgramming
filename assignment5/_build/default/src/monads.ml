...
let parse (c : char) : (bool, char) Stack_monad.t = 
  match c with
  | '(' -> Stack_monad.push '(' >>| fun () -> true
  | ')' -> Stack_monad.pop >>= fun top -> Stack_monad.return (Char.equal top '(')
  | _ -> Stack_monad.return true

let main_monadic (s : string) : (bool, char) Stack_monad.t = 
  let parse_and_accumulate (b : bool) (c : char) : (bool, char) Stack_monad.t =
    let%bind result = parse c in
    Stack_monad.return (b && result)
  in
  let%bind final_result = String.fold s ~init:(Stack_monad.return true) ~f:(fun acc c ->
    acc >>= fun b ->
    parse_and_accumulate b c
  ) in
  let%bind is_stack_empty = Stack_monad.is_empty in
  Stack_monad.return (final_result && is_stack_empty)
...
struct
  ...
  module T : Monad.Basic2 with type ('a, 'e) t = 'e Stack.t -> ('a, exn) result * 'e Stack.t =
    struct
      type ('a, 'e) t = 'e Stack.t -> ('a, exn) result * 'e Stack.t
  ...
  let run (x : ('a, 'e) t) : ('a, exn) result = match x [] with a, _ -> a
  ...
  let pop : ('e, 'e) t =
    fun (s : 'e Stack.t) ->
      match s with
      | [] -> Error (Failure "empty pop"), s
      | hd :: tl -> Ok hd, tl

  let is_empty : (bool, 'e) t =
    fun (s : 'e Stack.t) ->
      Ok (List.length s = 0), s
end

open Exception_stack
open Exception_stack.Let_syntax

let parse_more (c : char) : (unit, char) Exception_stack.t =
  match c with
  | '(' -> Exception_stack.push '('
  | ')' ->
    Exception_stack.pop >>= fun top ->
    if Char.equal top '(' then
      Exception_stack.return ()
    else
      failwith "Mismatched parentheses"
  | _ -> Exception_stack.return ()

  let main_more_monadic (s : string) : (bool, char) Exception_stack.t =
    let%bind () = String.fold s ~init:(Exception_stack.return ()) ~f:(fun acc c ->
      acc >>= fun () ->
      parse_more c
    ) in
    let%bind is_stack_empty = Exception_stack.is_empty in
    Exception_stack.return is_stack_empty  

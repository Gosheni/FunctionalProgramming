...
let () =
  let target_dir = 
    ...
    | _ -> Core_unix.getcwd ()
  in
  let keyword_counts = Utils.count_keywords_in_directory target_dir in
  let result = Utils.sexp_of_stat keyword_counts in
  let answer = Sexp.to_string result in
  Stdio.print_endline answer
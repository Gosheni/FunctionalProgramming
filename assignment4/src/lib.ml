...
module MakeDistribution (Item : Map.Key) (R : MyRandom) : Distribution with type item = Item.t = struct
  module Item_List = struct
    type t = Item.t list [@@deriving compare, sexp]
  end

  module ContextMap = Map.Make(Item_List)

  type item = Item.t
  type t = item Bag.t option ContextMap.t

  let get_ngram (sequence : item list) n =
    if n <= 0 || List.length sequence <= n then []
    else List.take sequence n

  let make_distribution (sequence : item list) ~n =
    let rec build_distribution (dist : 'a ContextMap.t) (sequence : item list) =
      let ngram = get_ngram sequence (n-1) in
      if List.is_empty ngram then dist
      else
        let context_bag = Option.value (Map.find dist ngram) ~default:(Some (Bag.create())) in
        match context_bag with 
        | Some v ->
          let updated_v = Bag.of_list ((Bag.to_list v) @ [List.nth_exn sequence (n-1)]) in
          let updated_dist = Map.update dist ngram ~f:(fun _ -> Some updated_v) in
          build_distribution updated_dist (List.tl_exn sequence)
        | None -> dist
    in
    build_distribution ContextMap.empty sequence
    
  let sample_random_sequence (dist : t) (initial_ngram : item list) ~n ~k =
    let rec sample_sequence (sequence : item list) (ngram : item list) =
      if List.length sequence >= k then sequence
      else
        let context_bag = Option.value (Map.find dist ngram) ~default:None in
        match context_bag with 
        | Some v ->
          let ngram_list = Bag.to_list v in
          let random_index = R.int (List.length ngram_list) in
          let next_context = List.nth_exn ngram_list random_index in
          let new_sequence, new_ngram = sequence @ [next_context], (List.drop ngram 1) @ [next_context] in
          sample_sequence new_sequence new_ngram        
        | None -> sequence
    in
    sample_sequence initial_ngram (List.drop initial_ngram (List.length initial_ngram - n + 1))
  
  let equal_test (dist : t) (ngram : item list) (bag : item Bag.t) =
    let rec compare_bag_elements (bag1 : item list) (bag2 : item list) =
      match (bag1, bag2) with
      | ([], []) -> true
      | (e1 :: tl1, e2 :: tl2) ->
        if Item.compare e1 e2 = 0 then compare_bag_elements tl1 tl2
        else false
      | _ -> false
    in
    let ngram_bag = Option.value (Map.find dist ngram) ~default:None in 
    match ngram_bag with 
    | Some v ->  
      compare_bag_elements (Bag.to_list bag) (Bag.to_list v)
    | None -> Bag.is_empty bag
    
    
  let to_list (dist : t) = 
    Map.fold dist ~init:[] ~f:(fun ~key ~data acc -> (key, data) :: acc)
    
  let equal_list_test (l1 : (item list * item Bag.t option) list) (l2 : (item list * item Bag.t option) list) : bool =
    let equal_lists list1 list2 =
      List.length list1 = List.length list2 &&
      List.for_all2_exn list1 list2 ~f:(fun elem1 elem2 -> (Item.compare elem1 elem2) = 0)
    in    
    if List.length l1 <> List.length l2 then false
    else
      List.for_all2_exn l1 l2 ~f:(fun (item1, bag1_opt) (item2, bag2_opt) ->
        equal_lists item1 item2 &&
        match (bag1_opt, bag2_opt) with
        | (Some bag1, Some bag2) ->
          equal_lists (Bag.to_list bag1) (Bag.to_list bag2)
        | (None, None) -> true
        | _ -> false
      )
